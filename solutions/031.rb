module Graphics
  ...
  class Point
    ...
    def *(multiplier)
      Point.new x * multiplier, y * multiplier
    end
  end
  ...
  class Line
    ...
    def rasterize_on(canvas)
      # step_count    = [(to.x - from.x).abs, (to.y - from.y).abs].max + 1
      # delta         = (to - from) / step_count.to_r

      # 0.upto(step_count).map { |step| delta * step }.each do |current_delta|
      #   current_point = from + current_delta
